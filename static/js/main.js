...
const events = () => {
    ...
    $('table.paginated').each(function() {
        ...
        $table.bind('repaginate', function() {
            ...
            $(this).parents('.table-wrapper').find('.page-number').addClass('hide')
            $(this).parents('.table-wrapper').find(`.page-number:nth-child(${currentPage+1})`).removeClass('hide');
        });
        ...
        function btnDisabling(btn, isToggled){
            $(btn).siblings().attr("disabled", false); //enable sibling btn
            if(isToggled){
        ...
        for (var page = 0; page < numPages; page++) {
            numTo = (1+page)*numPerPage <= numRows?(1+page)*numPerPage : numRows; 
            $(`<p class="page-number">${page*numPerPage+1}-${numTo} of ${numRows}</p>`).appendTo($pager).addClass('hide');
        }

        $(`<span id="back" class="icon public bgDark radius0 rotate180 material-icons" ${currentPage == 0? 'disabled':''}>arrow_forward_ios</span>`).bind('click', function(){
            if(currentPage != 0) {
                currentPage--;
                btnDisabling($(this), currentPage == 0)
            }
        }).appendTo($pager);
        $(`<span id="forward" class="icon public bgDark radius0 material-icons" ${numPages == currentPage+1? 'disabled':''}>arrow_forward_ios</span>`).bind('click', function(){
            if(numPages != currentPage+1){
                currentPage++;
                btnDisabling($(this), numPages == currentPage+1)
            }
        }).appendTo($pager);
            
        
        $(this).parents('.table-wrapper').prepend($pager).find('.page-number:first').removeClass('hide');
...
// in pagination make the max num = row num
// check why cant add 2 tables correctly
//  on each .pagination it recreate the first element